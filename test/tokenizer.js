...
import { isArabicChar, isWhiteSpace } from "../src/char.js";
  ...
  describe("tokenize", function () {
    ...
    describe("registerContextChecker", function () {
      ...
      before(function () {
        ...
        tokenizerContextual.registerContextChecker(
          "word",
          wordStartCheck,
          wordEndCheck,
        );
      ...
      it("should retrieve found ranges of a registered context (word)", function () {
        const whitespaceRanges = tokenizerContextual.getContextRanges(
          "whitespace",
        );
    ...
    describe("insert, delete, and replace a token or a range of tokens", function () {
      ...
      function getAan() {
        ...
        const AanTokens = AanRanges.map((range) =>
          tokenizer.getRangeTokens(range)
        );
        return AanTokens.map((tokens) =>
      ...
      it("should replace a range of tokens with tokens list", function () {
        ...
        rangesBefore.forEach((range) => {
          ...
          let newTokens = tokenizer.getRangeTokens(range).map(() =>
            new Token("…")
          );
      ...
      it("should compose a set of operations", function () {
        ...
        tokenizer.composeRUD([
          ...
          [
            ...
            11,
            null,
            Array.from("follower be a student!").map((c) => new Token(c)),
