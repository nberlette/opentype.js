...
describe("featureQuery.js", function () {
  ...
  describe("getScriptFeature", function () {
    ...
    it("should return a feature lookup tables", function () {
      /** arab */
      const initFeature = query.arabic.getFeature({
        tag: "init",
        script: "arab",
      });
      ...
      assert.deepEqual(
        initFeatureLookups[0],
        arabicFont.tables.gsub.lookups[7],
      );
      /** latin */
      const ligaFeature = query.latin.getFeature({
        tag: "liga",
        script: "latn",
      });
      ...
      assert.deepEqual(
        ligaFeatureLookups[0],
        latinFont.tables.gsub.lookups[35],
      );
    });
    it("should return lookup subtables", function () {
      ...
      const initFeatureLookups = query.arabic.getFeatureLookups(initFeature);
      const initLookupSubtables = query.arabic.getLookupSubtables(
        initFeatureLookups[0],
      );
      assert.deepEqual(
        initLookupSubtables.length,
        arabicFont.tables.gsub.lookups[7].subtables.length,
      );
      /** latin */
      const ligaFeature = query.latin.getFeature({
        tag: "liga",
      ...
      const ligaFeatureLookups = query.latin.getFeatureLookups(ligaFeature);
      const ligaLookupSubtables = query.latin.getLookupSubtables(
        ligaFeatureLookups[0],
      );
      assert.deepEqual(
        ligaLookupSubtables.length,
        latinFont.tables.gsub.lookups[35].subtables.length,
      );
    });
    it("should return subtable lookup method", function () {
      ...
      const initSubsType = query.arabic.getSubstitutionType(
        initLookupTable,
        initSubtable,
      );
      const initLookupFn = query.arabic.getLookupMethod(
        initLookupTable,
        initSubtable,
      );
      ...
      const ligaSubsType = query.latin.getSubstitutionType(
        ligaLookupTable,
        ligaSubtable,
      );
      const ligaLookupFn = query.latin.getLookupMethod(
        ligaLookupTable,
        ligaSubtable,
      );
    ...
    it("should find a substitute - single substitution format 1 (11)", function () {
      const feature = query.arabicChanga.getFeature({
        tag: "init",
        script: "arab",
      });
      const featureLookups = query.arabicChanga.getFeatureLookups(feature);
      const lookupSubtables = query.arabicChanga.getLookupSubtables(
        featureLookups[0],
      );
      const substitutionType = query.arabicChanga.getSubstitutionType(
        featureLookups[0],
        lookupSubtables[0],
      );
      assert.equal(substitutionType, 11);
      const lookup = query.arabicChanga.getLookupMethod(
        featureLookups[0],
        lookupSubtables[0],
      );
    ...
    it("should find a substitute - single substitution format 2 (12)", function () {
      ...
      const lookupSubtables = query.arabic.getLookupSubtables(
        featureLookups[0],
      );
      const substitutionType = query.arabic.getSubstitutionType(
        featureLookups[0],
        lookupSubtables[0],
      );
      assert.equal(substitutionType, 12);
      const lookup = query.arabic.getLookupMethod(
        featureLookups[0],
        lookupSubtables[0],
      );
