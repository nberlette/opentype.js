...
describe("types.js", function () {
  ...
  it("handles the range guard for FLOAT (16.16) representation", function () {
    ...
    assert.doesNotThrow(function () {
      encode.FLOAT(MIN_16_16);
    }, error);
    assert.doesNotThrow(function () {
      encode.FLOAT(MAX_16_16);
    }, error);
    assert.throws(function () {
      encode.FLOAT(MIN_16_16 - 0.001);
    }, error);
    assert.throws(function () {
      encode.FLOAT(MAX_16_16 + 0.001);
    }, error);
  });
  ...
  it("can handle UTF16", function () {
    ...
    assert.equal(
      decode.UTF16(unhex("DE AD D8 3D DC 04"), 2, 4),
      "\uD83D\uDC04",
    );
  ...
  it("can encode VARDELTAS", function () {
    ...
    assert.equal(
      e(new Array(64).fill(127)),
      "3F " + (new Array(64).fill("7F")).join(" "),
    );
    assert.equal(
      e(new Array(65).fill(127)),
    ...
    assert.equal(
      e(new Array(64).fill(0x1122)),
      "7F " + (new Array(64).fill("11 22")).join(" "),
    );
    assert.equal(
      e(new Array(65).fill(0x1122)),
