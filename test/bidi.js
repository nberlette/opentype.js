...
describe("bidi.js", function () {
  ...
  before(function () {
    ...
    bidiScheherazade.registerModifier(
      "glyphIndex",
      null,
      (token) => arabicFont.charToGlyphIndex(token.char),
    );
    ...
    bidiFira.registerModifier(
      "glyphIndex",
      null,
      (token) => latinFont.charToGlyphIndex(token.char),
    );
    const latinFeatures = [{
      script: "latn",
      tags: ["liga", "rlig"],
  ...
  describe("arabic contexts", function () {
    ...
    it("should match mixed arabic sentence", function () {
      ...
      const sentences =
        ranges.map((range) => arabicTokenizer.rangeToText(range))[0];
  ...
  describe("getBidiText", function () {
    it("should adjust then render layout direction of bidi text", function () {
      const bidiText = bidiScheherazade.getBidiText(
        "Be kind, فما كان الرفق في شيء إلا زانه ، ولا نزع من شيء إلا شانه",
      );
      assert.equal(
        bidiText,
        "Be kind, هناش الإ ءيش نم عزن الو ، هناز الإ ءيش يف قفرلا ناك امف",
      );
    });
  });
  describe("applyFeatures", function () {
    it("should apply arabic presentation forms", function () {
      ...
      assert.deepEqual(
        PeaceForms,
        [null, "init", "medi", "medi", "fina", null].reverse(),
      );
    });
