...
import { average, log2 } from "../util.js";
import * as check from "../check.js";
import * as cmap from "./cmap.js";
import * as cff from "./cff.js";
import * as head from "./head.js";
import * as hhea from "./hhea.js";
import * as hmtx from "./hmtx.js";
import * as ltag from "./ltag.js";
import * as maxp from "./maxp.js";
import * as _name from "./name.js";
import * as os2 from "./os2.js";
import * as post from "./post.js";
import * as gsub from "./gsub.js";
import * as meta from "./meta.js";
import * as colr from "./colr.js";
import * as cpal from "./cpal.js";
import * as fvar from "./fvar.js";
import * as stat from "./stat.js";
import * as avar from "./avar.js";

/**
 * Convert the font object to a SFNT data structure. This structure contains
 * all the necessary tables and metadata to create a binary OTF file.
 *
 * @param {import("../font.js").Font} font - The input font object.
 */
export function fontToTable(font) {
  const xMins = [];
  const yMins = [];
  const xMaxs = [];
  const yMaxs = [];
  ...
  const os2Table = os2.make(Object.assign({
    ...
    sxHeight:
      metricsForChar(font, "xyvw", { yMax: Math.round(globals.ascender / 2) })
        .yMax,
  ...
  if (!postScriptName) {
    postScriptName = englishFamilyName.replace(/\s/g, "") + "-" +
      englishStyleName;
  }
  ...
  if (!names.unicode.preferredFamily) {
    names.unicode.preferredFamily = fontNamesUnicode.fontFamily ||
      fontNamesMacintosh.fontFamily || fontNamesWindows.fontFamily;
  }

  if (!names.macintosh.preferredFamily) {
    names.macintosh.preferredFamily = fontNamesMacintosh.fontFamily ||
      fontNamesUnicode.fontFamily || fontNamesWindows.fontFamily;
  }

  if (!names.windows.preferredFamily) {
    names.windows.preferredFamily = fontNamesWindows.fontFamily ||
      fontNamesUnicode.fontFamily || fontNamesMacintosh.fontFamily;
  }

  if (!names.unicode.preferredSubfamily) {
    names.unicode.preferredSubfamily = fontNamesUnicode.fontSubFamily ||
      fontNamesMacintosh.fontSubFamily || fontNamesWindows.fontSubFamily;
  }

  if (!names.macintosh.preferredSubfamily) {
    names.macintosh.preferredSubfamily = fontNamesMacintosh.fontSubFamily ||
      fontNamesUnicode.fontSubFamily || fontNamesWindows.fontSubFamily;
  }

  if (!names.windows.preferredSubfamily) {
    names.windows.preferredSubfamily = fontNamesWindows.fontSubFamily ||
      fontNamesUnicode.fontSubFamily || fontNamesMacintosh.fontSubFamily;
  }

  // we have to handle fvar before name, because it may modify name IDs
  const fvarTable = font.tables.fvar
    ? fvar.make(font.tables.fvar, font.names)
    : undefined;

  const languageTags = [];
  const nameTable = _name.make(names, languageTags);
  const ltagTable = languageTags.length > 0
  ...
  // The order does not matter because make() will sort them.
  const tables = [
    headTable,
    hheaTable,
    maxpTable,
...
export function make(tables) {
  const sfnt = new Table("sfnt", [
